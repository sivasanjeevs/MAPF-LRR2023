...
class LiveMAPFVisualizer:
    """
    A live, time-accurate visualizer for MAPF that dynamically updates
    all information based on the current visualization timestep.
    """
    ...
    def __init__(self, map_file: str, json_file: str):
        ...
        self.task_events: Dict[int, List[Dict]] = {} # Stores event history for each task
    ...
    def _calculate_path_from_actions(self, start_row: int, start_col: int, start_orientation: int, actions_str: str) -> List[Tuple[int, int, int]]:
        ...
        for action in actions:
            if action == 'R': orientation = (orientation + 1) % 4
            elif action == 'C': orientation = (orientation - 1 + 4) % 4
            elif action == 'F':
                if orientation == 0: row -= 1
                elif orientation == 1: col += 1
                elif orientation == 2: row += 1
                elif orientation == 3: col -= 1
            path.append((row, col, orientation))
        return path
    ...
    def _load_data_from_json(self):
        try:
            ...
            # Process events to build a history for each task
            ...
            # Sort events for each task by timestep
    ...
    def get_task_state_at_timestep(self, task_id: int, timestep: int) -> Tuple[str, Dict]:
        """Gets the state of a task at a specific timestep."""
        last_event = None
        ...
        if last_event:
            return last_event['type'], last_event
    ...
    def _draw_agents(self):
        ...
        interpolation_factor = self.current_timestep - frame_idx
        for i, path in enumerate(self.agents_paths):
            if not path: continue
            color = AGENT_COLORS[i % len(AGENT_COLORS)]
            current_pos_idx, next_pos_idx = min(frame_idx, len(path) - 1), min(frame_idx + 1, len(path) - 1)
            r1, c1, orientation = path[current_pos_idx]
            r2, c2, _ = path[next_pos_idx]
            interp_r, interp_c = r1 + (r2 - r1) * interpolation_factor, c1 + (c2 - c1) * interpolation_factor
            center_x, center_y = self.margin + interp_c * self.cell_size + self.cell_size / 2, self.margin + interp_r * self.cell_size + self.cell_size / 2
    ...
    def _draw_legend(self):
        ...
        # Calculate tasks finished at current timestep
        tasks_finished_now = 0
        for task_id in self.tasks:
             state, _ = self.get_task_state_at_timestep(task_id[0], display_frame)
             if state == "finished":
                 tasks_finished_now += 1
